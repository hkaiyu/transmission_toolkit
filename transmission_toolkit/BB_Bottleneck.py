...
def bb_input_data(donor, recip, min_read_depth=0, max_AF=1, parse_type="biallelic", store_ref=True, weighted=False):
    """
    Stores info from parsing VCF files to dictionary.
    """
    donor_data, donor_masks = extract_lfv(donor, min_read_depth=min_read_depth, max_AF=max_AF, parse_type=parse_type, store_ref=store_ref)
    recip_data, recip_masks = extract_lfv(recip, min_read_depth=min_read_depth, max_AF=max_AF, parse_type=parse_type, store_ref=store_ref)
